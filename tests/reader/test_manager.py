...
class TestManager:
    ...
    def test_register_no_id(self, manager):
        ...
        """
        with pytest.raises(Manager.NoSchemaIdError, match=r"Schema must have an 'id' attribute, which is not None."):
    ...
    def test_register_existing_id(self, manager):
        ...
        with pytest.raises(Manager.SchemaIdExistsError, match=r"Schema with uri .* already exists."):
    ...
    def test_from_rad(self):
        ...
        assert len(manager) == len(manager._tag_to_uri)
