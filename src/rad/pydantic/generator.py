...
def create_code(file: Result, version: str) -> str:
    header = (
        dedent(
            f"""
            # Generated by RAD using generator based on datamodel-codegen
...
def write_files(path: Path, result: dict[tuple[str], Result], version: str | None = None) -> None:
    version = version or "1.0.0"
    for name, file in result.items():
        with make_file_path(path, name).open("w") as f:
            f.write(create_code(file, version))


def generate_files(suffix: str, write_path: Path):
    schema_path = get_rad_schema_path(suffix)
    parsed_results = create_rad_schema_parser(schema_path).parse()
    write_files(write_path, parsed_results)
...
if __name__ == "__main__":
    ...
    generate_files("schemas", write_path)
